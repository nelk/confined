...
Light::Light(LightType type, const glm::vec3& colour, const glm::vec3& position, const glm::vec3& direction, float spread)
  : type(type), colour(colour), ambientColour(0, 0, 0), position(position), direction(direction), falloff(1, 0, 0), spread(spread) {}
...
Light* Light::spotLight(const glm::vec3& colour, const glm::vec3& direction, float spread) {
  return new Light(SPOT, colour, glm::vec3(0, 0, 0), direction, spread);
}
