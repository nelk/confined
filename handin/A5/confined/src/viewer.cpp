...
bool Viewer::initGL() {
  ...
  if (quad_programId == 0 || depthProgramId == 0 || programId == 0) {
    return false;
  }
...
void Viewer::run() {
  ...
  GLuint material_kd = glGetUniformLocation(programId, "material_kd");
  ...
  do {
    ...
    // Use depth pass shader.
    glUseProgram(depthProgramId);

    lightTime += 0.02;
    lightDir[1] = std::sin(lightTime);
    lightDir[2] = std::cos(lightTime);
